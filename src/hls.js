...
export default class Hls extends Observer {
  ...
  attachMedia (media) {
    ...
    this.media = media;
    // 通知各个controller媒体已经被成功附上，各自对应的处理函数开始工作。
  ...
  loadSource (url) {
    ...
    // 这里触发播放列表的解析，这里必须要先进行
    // 重要：这里的工作消息路径大概是 MANIFEST_LOADING(触发加载播放列表) -> MANIFEST_LOADED(m3u8文件解析播放列表)
    // -> MANIFEST_PARSED(音视频以及清晰度抽象分层) -> 开始工作

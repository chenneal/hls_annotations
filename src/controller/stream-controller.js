...
class StreamController extends BaseStreamController {
  ...
  doTick () {
    ...
    case State.WAITING_LEVEL:
      // 这里循环check清晰度信息是否已经获取，如果加载完毕，切换控制器状态。
    ...
    case State.FRAG_LOADING_WAITING_RETRY:
      // 同样是循环等待ts重试，只有超时或者固定的一些情况才切换控制器状态。
  ...
  // 这里比较逗，作者把定时任务的主要处理逻辑都放在idle状态处理，其实可以完全可以换个状态名字。
  _doTickIdle () {
    const hls = this.hls,
      config = hls.config,
      media = this.media;
    ...
    // 如果 media 还没有 attach ，直接跳过，这个 prefetch 还不是太明白。
    ...
    // 计算最大的缓冲区域，由物理大小和时间大小共同约束。
    // maxBufferSize 代表物理大小，maxBufferLength 代表时长。
  ...
  _fetchPayloadOrEos (pos, bufferInfo, levelDetails) {
    ...
    if (!frag) {
      // 这里根据二分查找去查找需要加载的 ts ，大概率是下一个 ts 。
  ...
  */
  ...
  onMediaAttached (data) {
    ...
    // 监听3个事件，其中seeking和seeking已经在父类中定义好了。
  ...
  _seekToStartPos () {
    ...
    if (currentTime !== startPosition) {
      ...
      // 注意这个操作会将video标签的媒体seek到指定位置，是一个实操作。
